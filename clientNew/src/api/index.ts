...
export const POSTWithParams = async (url: string, body: any, params: any, token: string) => {
    const { data } = await axios.post(`${endUrl}${url}`,
        body,
        {
            headers: { Authorization: `Bearer ${token}` },
...
export const PUTWithParams = async (url: string, body: any, params: any, token: string) => {
    const { data } = await axios.put(`${endUrl}${url}`,
        {
            ...body
        },

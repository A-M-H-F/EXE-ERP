...
import { App, Button, Form, Input, Modal, Select, Spin, message } from 'antd'
...
import { CloudServerOutlined, DollarOutlined, PlusOutlined, WifiOutlined } from '@ant-design/icons'
...
const AddNewInternetService = () => {
    ...
    const handleAddInternetService = async () => {
        // name
        ...
        if (serviceLength) {
            messageApi.open({
                type: 'error',
                content: 'Service must be at least 3 characters long'
            })
            return
        }
        ...
        try {
            ...
            const { data } = await apiService.POST('/internet-service', body, token)
            ...
            setInternetServiceInfo(initialState)
            handleCancel()
            setAdding(false)

            const updatedList = [...internetServicesList, newInternetService]
    ...
    return (
        <>
            ...
            >
                <Form
                    layout='vertical'
                    onFinish={handleAddInternetService}
                >
...
export default AddNewInternetService